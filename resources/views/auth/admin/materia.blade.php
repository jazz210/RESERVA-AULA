...
<div class="block mx-auto my-12 p-8   w-1/3 border-gray-400 rounded-lg shadow-lg" style="background-color:rgba(50, 156, 192, 0.5)">
    ...
    <h1 class="text-3xl text-center fond-bold ">Añadir materia</h1>
    <form class="mt-4" method="POST" action="">
        @csrf

        <input type="name" class="border border-gray-200 rounded-md bg-gray-200 w-full
        text-lg placeholder-gray-900 p-2 my-2 focus:bg-white" placeholder="Nombre Materia" id="name" name="name">
        @error('name')
    ...
    @enderror
        ...
        text-lg placeholder-gray-900 p-2 my-2 focus:bg-white" placeholder="Codigo materia" id="email" name="email">
        @error('email')
      <p class="border border-red-500 rounded-md bg-red-100 w-full
      text-red-600 p-2 my-2">* {{ $message }}</p>
    @enderror
